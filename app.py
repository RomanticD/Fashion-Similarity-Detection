...
# import numpy as np
# import os
# from groundingdino_handler import load_groundingdino_model, get_transform, process_image, detect_clothes
#
# # 设置页面
# st.title('服装款式相似性检测')
#
...
#     img = Image.open(upload_img_file).convert("RGB")
#     image = np.asarray(img)
#     segments = process_image(image, transform)
#     FRAME_WINDOW.image(img, channels='BGR')
#
# # 触发服装检测
# if st.button('检测页面中的服装'):
#     try:
#         st.session_state.clothes_bboxes = detect_clothes(model, transform, segments, TEXT_PROMPT, BOX_THRESHOLD)
#         st.write("检测到的服装区域：")
#         for idx, bbox in enumerate(st.session_state.clothes_bboxes):
#             st.image(bbox, caption=f"服装 {idx + 1}")
#     except Exception as e:
...
#             bbox_image.save(tmp_image_path)
#
#             # 调用 mmfashion_retrieval.py 进行相似款检测
#             os.system(f"python3 mmfashion/tools/test_retriever.py --input {tmp_image_path}")
#
#             # 显示相似款图像
#             output_image_path = './output.png'
